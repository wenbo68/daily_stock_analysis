...
- **`DimensionCard` and `PayloadTable`** — also defined inside this file. They draw
  the four data cards (technicals / fundamentals / macro / positioning; old stored
  runs show the retired news-sentiment card instead of positioning) and their
...
| `components/tiered-alt/AltRunForm.tsx` | Section 1: write-only fields (ticker / capital / risk / reward / tier) whose choices land as removable `Label: value` pills below — colored by POSITION in the showplayer palette starting at red (stone skipped, owner decision 2026-07-21): red ticker, orange capital, amber risk, lime reward, emerald tier, and the Start pill wears the next slot (sky). Tier, risk and reward show default pills on entry; picking a ticker auto-fills capital (in the ticker market's currency — see `altCurrency.ts`), and removing the ticker removes capital with it. Reward (`Reward: ratio`) is the reward-to-risk ratio the plan aims for: dropdown 1.5/2/3 or free text (> 1, ≤ 10), default 2, remembered like capital/risk; its pill reads `Reward: 2×`. The ownership input is gone — deferred to the future portfolio feature (the API still accepts `ownership`; this page never sends it, so actions always come from ownership = 0). The tier options are named `1: preliminary analysis` and `2: deep analysis`. Start requires all five fields and explains in a popup otherwise; setting capital before a ticker also gets a popup. Picking an already-picked dropdown option clears it, same as clicking its pill. The tier picker offers 1–2 only (tier 3 retired; the backend rejects depth 3). |
...
| `components/tiered-alt/AltLevels.tsx` | The plan levels as a Computed/Adjusted table (columns: ideal entry, stop loss, target — the backup entry is retired and old runs' stored backup no longer renders; untouched levels say `keep`). Clicking a computed number opens a modal titled `<level>: formula` — formula in words (variables italic, human names like `ideal entry`), plugged-in numbers (each linking to a technicals row or another computed cell), result; all three lines share one font/size/spacing and never wrap (the modal widens to fit). The stored prose phrases are expanded into real values: `support candidates` becomes the run's actual supports (sma 20/60, swing lows 20/60, round level) and `nearest overhead resistance` becomes `nearest of (swing high 20, swing high 60, 52w high)` — every value linked to its technicals row, except the round level (computed on the spot), whose number opens a small how-computed modal. Clicking an adjusted number opens `<level>: adjustment` — the AI's reason and references. Old runs without the audit trail fall back to a single row of stored values. |
| `components/tiered-alt/AltDimensions.tsx` | The four data cards, alt-styled. The technicals payload renders in named sections — Price & trend, Momentum, Volatility & range, Price structure (swing lows/highs, 52-week range), Volume, Data & score — with every current backend key mapped, so the trailing `Other` section never appears on real data (it survives only as a crash-guard for future unmapped keys). Each card ends in a single `Sources` list — non-link sources first, links after (shown as their URL, not their headline), every entry keeping the `[n]` number its inline marks refer to. |
