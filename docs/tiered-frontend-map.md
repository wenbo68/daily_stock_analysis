...
| `components/tiered-alt/AltDebateTree.tsx` | The v5 debate tree: one tree, a four-step selector (1 defender argument → 2 attacker check → 3 defender response → 4 judge ruling; step 4 is the default full view). Items grouped by dimension, each with its id, a bullish/bearish tag, an `added by attacker` tag on additions, citation chips (payload paths scroll-flash their metric row; `sentiment.citation:N` scroll-flashes source N in the sentiment report's sources list — no external jump), and at step 4 a `counted 1/1 | 0/1 | 0/0` badge tinted by outcome. Under each item: attacker checks (sky), defender responses (accepted / rejected with the failing check's reason), judge rulings (violet; attack upheld/overruled, valid/invalid, real/bogus). The Scores block at the bottom shows initial → adjusted position score, then the weight ledger (`correct keeps / (correct keeps + errors)` = N/D = value) and the final formula `5 + weight × (adjusted − 5)` in the shared words/plugged/result shape, ending in the `below 4 sell · 4–6 hold · above 6 buy` verdict block. Voided runs render the partial tree without the Scores block. |
