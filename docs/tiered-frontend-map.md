...
| `components/tiered-alt/AltDebateTree.tsx` | The debate tree, four generations. v8 (`format: 8`) is the evidence vote: a three-step selector (1 evidence list → 2 votes → 3 final score; step 3 default), bullets grouped by dimension with plain colored bullish/bearish text, cited display values underlined inline (located by the same comma-tolerant, digit-bounded pattern the backend verifies with), sentiment sources as trailing [N] hyperlinks that scroll-flash source N. Under each bullet: a `listed independently by both analysts — confirmed` line (2-0 at birth) or its vote lines — `checker · vote · valid/invalid` (sky) and `deciding vote · …` (violet), reasons rendered with the same inline value links and trailing [N]s. No counted/excluded pills: a bullet struck by the code citation check is crossed out at every step; a bullet voted out is crossed out at the final step. The Scores block shows the initial (merged) score labeled `average of N dimensions`, then the final formula `10 × bullish / total` per dimension, the averaged result, and the verdict bands — no adjusted line. v7/v6 runs keep their defender/attacker/judge threads (single-axis / two-axis), v6 keeps amber code value-check lines and counted/excluded badges, stored v5 runs keep chips, real/bogus words and the weight-formula block. Voided runs render the partial tree without the Scores block. |
