...
| `components/tiered-alt/AltRunForm.tsx` | Section 1: write-only fields (ticker / tier / capital / risk) whose choices land as removable `Label: value` pills below, plus the indigo Start pill. Tier defaults to 1 and shows its pill on entry. Nothing runs on change — only on Start. Picking an already-picked dropdown option clears it, same as clicking its pill. |
| `components/tiered-alt/AltRunHistory.tsx` | Section 2: filters (ticker / date Min–Max / verdict / tier / shares Min–Max) that apply the moment they're entered and show as pills; ticker, verdict and tier are multi-pick dropdowns (ticker lists every ticker seen in history). Then the run rows (10 per page: ticker, date `yyyy/mm/dd, hh:mm`, tier, verdict, shares) that expand inline into the full report, paged by the showplayer-style numbered page buttons. |
