...
| `components/tiered-alt/AltDebateTree.tsx` | The debate tree, two renderers. v8/v9/v10 (`VoteTree`) is the evidence vote inside a collapsed `Transcript` foldable: a numbered `How this works` list first (noting that a bullet with NO marks was listed by both AIs — already agreed), then per-dimension groups headed `Technicals: 7 bullish, 4 bearish` (green/red words, counting only surviving bullets so the headers sum to the score's fraction), each bullet a two-column grid row (hanging indent on wrap) of id + colored bullish/bearish word + one clickable ✓/✗ mark per check (the code's ✗ on citation-struck bullets). A mark opens an AltModal titled `1st/2nd check result: valid/invalid` (`code check result: invalid` for struck) with the reasoning — inline value links and trailing [N] source links. Crossed out = not counted (struck or voted out). The Scores block is `final position score: 10 × bullish / total items` plugged in (formula hidden for stored format-8 runs whose score was a per-dimension mean) — no verdict-bands block. v10 (weighted votes) adds: a clickable `w 2.5` weight badge per bullet (the median of every voter's 1-3 importance rating — its modal explains the rubric and lists the author ratings), the voter's own rating inside each ✓/✗ vote modal, an extra how-it-works line, and the weighted Scores block `10 × bullish weight / total weight`. v5/v6/v7 stored runs keep the `RoleDebateTree` renderer: the 4-step selector, defender/attacker/judge threads, chips/weight (v5), amber value-check lines + badges (v6), single-axis threads (v7). Voided runs render the partial tree without the Scores block. |
