...
| `components/tiered-alt/AltDebateTree.tsx` | The v5/v6/v7 debate tree: one tree, a four-step selector (1 defender argument → 2 attacker check → 3 defender response → 4 judge ruling; step 4 is the default full view). Items grouped by dimension with plain colored bullish/bearish text (no pills) and `·` separators throughout. v7 claims underline exactly the cited display value inside the sentence (located by the same comma-tolerant, digit-bounded pattern the backend verifies with); sentiment links underline their words; payload refs scroll-flash their metric row, `citation:N` scroll-flashes source N in the sentiment report's sources list. Bullets whose citations code could not fix render struck through — no thread, no badge, counted nowhere. The v7 thread is a single logic axis: attacker check (sky), defender response as one check line, judge line (violet; attack rulings show attack upheld/overruled). v6 runs keep their word-underlines, amber code value-check lines, two-axis threads and mismatch reds; stored v5 runs keep chips and real/bogus words. Additions open with an `attacker · newly added` line. Step 4 badges: `counted`/`excluded` (v5 keeps the old `counted N/N` ledger). The Scores block shows initial → adjusted, each labeled `average of N dimensions`, then the final formula `10 × bullish / total` per dimension with each dimension's line, the averaged result, and the `below 4 sell · 4–6 hold · above 6 buy` verdict block; stored v5 runs keep the weight-formula block. Voided runs render the partial tree without the Scores block. |
