...
| `components/tiered-alt/AltDebateTree.tsx` | The v5/v6 debate tree: one tree, a four-step selector (1 defender argument → 2 attacker check → 3 defender response → 4 judge ruling; step 4 is the default full view). Items grouped by dimension with plain colored bullish/bearish text (no pills) and `·` separators throughout. v6 claims render the cited words themselves as underlined tinted links — payload refs scroll-flash their metric row, `citation:N` scroll-flashes source N in the sentiment report's sources list; mismatched values render red; no trailing chips (stored v5 runs keep chips). Additions open with an `attacker · newly added` line. Under each item: the code's mechanical value-check line (amber) when a number failed, attacker checks (sky), defender responses as two check lines (the valid/invalid words tell the accept/reject story), judge rulings (violet) — on v6 additions the judge's own two checks, on v5 additions the stored real/bogus word. Step 4 badges: v6 `counted`/`excluded`, v5 the old `counted N/N` ledger. The v6 Scores block shows initial → adjusted (with bullish/bearish counts), then the final formula `10 × bullish / total` per dimension with each dimension's line, the averaged result, and the `below 4 sell · 4–6 hold · above 6 buy` verdict block; stored v5 runs keep the weight-formula block. Voided runs render the partial tree without the Scores block. |
