...
| 5. Tier 3 risk stress | **done** (2026-07-12) — `risk.py` + real `Tier3Stage` (code-validated tightened stop; multiplier enum {0, 0.5, 1} applied by `apply_size_multiplier`, wired to sizing in slice 6); 19 offline tests (259 tiered total). Live smoke (2026-07-12, paid Gemini key): full depth-3 AAPL run passed — risk judge ruled hold, size_multiplier 0.0 (on-enum), stop_advice keep, all 4 key risks evidence-anchored (`fundamentals.*`, `technicals.close`, `citation:1`); the degradation path (fallback to tier-2 direction on judge failure) had been verified live earlier under free-tier quota exhaustion |
| 6. Pipeline/API integration | **done** (2026-07-12, commit 1f10f2e) — `depth` 1\|2\|3 through `run_tiered_analysis` and `POST /api/v1/tiered/analyze`; sizing settings via `.env` `TIERED_SIZING_*` keys (off unless capital + risk fraction set) with per-run API override; tier-3 multiplier applied by code (0 shares recorded explicitly); `LlmUsageTracker` per-stage call/token counts; response additively gains `depth`/`final`/`tier2`/`tier3`/`sizing`/`llm_usage`; ledger logs the deepest tier + sized position. 35 new offline tests (294 tiered total). Live depth-3 AAPL run via the API verified: judge hold @ 0.70, risk multiplier 0.0, sizing refused `not_a_buy` (hold), usage 8 calls / ~35k tokens across adjuster+debate+risk, signal id 22 logged with tier 3 |
| 7. Web surface (incl. level modals) | **done** (2026-07-12, commit 2846f83) — depth selector + optional sizing inputs on the run form (per-run overrides, browser-remembered); level tiles per the two-modal spec (formula modal with plugged-in numbers linking to technicals rows; adjustment modal with reasoning + hyperlinked references, rejections visible); tier-2 debate card, tier-3 risk card, sizing card (sized/refused/off); final-verdict header, LLM usage line; ~100 en/zh strings with help popups; 14 component tests, lint + build green; old stored runs render unchanged (fallback tested). Verified served bundle + stored depth-3 run carries all UI fields |
