...
| 5. Tier 3 risk stress | **done** (2026-07-12) — `risk.py` + real `Tier3Stage` (code-validated tightened stop; multiplier enum {0, 0.5, 1} applied by `apply_size_multiplier`, wired to sizing in slice 6); 19 offline tests (259 tiered total). Live smoke: personas produced evidence-anchored takes; the judge call hit the Gemini free-tier daily quota (20 req/day) and the degradation path (fallback to tier-2 direction, quota error in warnings) verified live instead — re-smoke the happy path after quota reset or on a paid key |
