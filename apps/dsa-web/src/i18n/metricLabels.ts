...
const en: Record<string, MetricEntry> = {
  ...
  implied_report_move_pct: {
    ...
    blank: 'Shown only when the next report is within ~3 weeks (further out, option prices mostly reflect ordinary drift, not the report jump) — and it needs the report date plus usable at-the-money quotes on the first expiration after it. The mark next to the value carries the exact reason for this run.',
  },
...
const zh: Record<string, MetricEntry> = {
  ...
  implied_report_move_pct: {
    ...
    blank: '仅在下次财报距今约 3 周以内才显示（更远时期权价格主要反映日常波动，而非财报跳空），且需要财报日期和财报后第一个到期日的可用平值报价。本次运行的具体原因见数值旁的提示标记。',
  },
  report_move_ratio_implied_4q: {
